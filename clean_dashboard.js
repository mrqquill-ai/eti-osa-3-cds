...
// e.g. { p_pin: adminPin, p_batch_size: pendingBatchSize } -> { p_batch_size: pendingBatchSize }
content = content.replace(/{ p_pin: adminPin, /g, '{ ');
// e.g. { p_super_pin: adminPin, p_state_code: code ... } -> { p_state_code: code ... }
content = content.replace(/{ p_super_pin: adminPin, /g, '{ ');
// e.g. { p_pin: adminPin } -> (nothing, or just omit second arg)
content = content.replace(/, { p_pin: adminPin }/g, '');
content = content.replace(/, { p_super_pin: adminPin }/g, '');
